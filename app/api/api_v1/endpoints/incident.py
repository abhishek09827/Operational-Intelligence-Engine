                    ...
                    try:
                        ...
                        try:
                            ...
                            if len(analysis_list) > 0:
                                ...
                                severity = analysis_dict.get('severity') or analysis_dict.get('Severity') or analysis_dict.get('level') or log_entry.severity
                                log_entry.severity = severity
                                
                                log_entry.root_cause = str(report_output) if report_output else analysis_dict.get('root_cause', '')
                                
                                confidence_score = analysis_dict.get('confidence_score') or analysis_dict.get('confidence')
                                if confidence_score is not None:
                                    try:
                                        log_entry.confidence_score = float(confidence_score)
                                    except ValueError:
                                        log_entry.confidence_score = None
                                
                                # Format as visually appealing markdown for frontend (same as /analyze)
                                log_entry.description = format_analysis_as_markdown(analysis_list[0])
                                
                                log_entry.status = "Analyzed"
                                print(str(validation_output) + "val123")
                                print(str(report_output) + "rep123")
                                log_entry.thinking_process = "\n".join(crew_output)
                            ...
                            log_entry.root_cause = cleaned_result if 'cleaned_result' in locals() else cleaned_result
                            log_entry.status = "Analyzed (Invalid JSON)"
                            db.commit()
                            db.refresh(log_entry)
...
def list_incidents(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    ...
    List past incidents.
    """
    incidents = db.query(Incident).order_by(Incident.created_at.desc()).offset(skip).all()
    return incidents